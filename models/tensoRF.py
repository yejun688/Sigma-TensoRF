...
class TensorVMSplit(TensorBase):
    ...
    # 生成一个掩码张量 (mask tensor)，用于在神经网络计算中选取或遮掩特定的神经元或特征通道
    ...
    # 初始化一个由 SVD 分解获得的低秩 (low-rank) 表示,mask_rank,用于创建掩码的秩 (rank)
...
#  dens_mark: 一个布尔值，指示使用密度掩码 (True) 还是外观掩码 (False)。
